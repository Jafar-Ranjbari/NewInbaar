...
const ReportFinanacrdriver: React.FC<ReportFinanacrdriverProps> = ({ isVisible, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchPayments = async () => {
      setLoading(true);
      try {
        const data = await getPaymentsByDriverId(driverID);
        setAllPayments(data);
            ...
            <div className="flex justify-center items-center mb-6">
              ...
              {availableYears.map((year) => (
                <button
                  ...
                  className={`px-4 py-1.5 mx-1 rounded-full text-sm transition-colors duration-200 font-semibold ${selectedYear === year
              ...
              {FA_MONTHS.map((monthName, index) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm transition-colors duration-200 font-semibold flex-shrink-0 ${selectedMonth === monthValue
                      ? 'bg-blue-500 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
