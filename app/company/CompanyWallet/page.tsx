...
import { SmsPackageModal } from './SmsPackageModal';
...
const CompanyWallet: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      if (!comp) comp = await createCompany(currentUser!.id, CompanyType.REAL);
      setCompanyID(comp.id);

      const [wTxs, sTxs] = await Promise.all([
        getCompanyWalletTransactions(comp.id),
  ...
  const handleTopUp = async (amountRials: number) => {
    setActionLoading(true);
    try {
      const t = await createCompanyWalletTransaction(companyID!, amountRials, `شارژ حساب`);
      setTxs(prev => [...prev, t]);
      setModalTopUp(false);
    } catch (e) {
      alert("خطا در شارژ");
  ...
  const handleBuySms = async (pkg: any) => {
    setActionLoading(true);
    try {
      const wTx = await createCompanyWalletTransaction(companyID!, -pkg.priceRials, `خرید ${pkg.label}`);
      const sTx = await createSmsCreditTransaction(currentUser!.id, pkg.count, pkg.priceRials / 10, `خرید ${pkg.label}`);
      setTxs(prev => [...prev, wTx]);
      setSmsTxs(prev => [...prev, sTx]);
      setModalSms(false);
    } catch (e) {
      alert("خطا در خرید");
    } finally {
  ...
  return (
    <PageLayout title="کیف پول">
      ...
      <SmsPackageModal 
        ...
        onConfirm={handleBuySms} 
        walletBalanceRials={walletBalance} 
        isLoading={actionLoading} 
      />

      {/* Wallet Card */}
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden mb-8">
        <div className="relative z-10 flex flex-col items-center">
          <p className="text-blue-100 text-sm mb-2">موجودی فعلی</p>
          <h3 className="text-4xl font-black">{(walletBalance / 10).toLocaleString('fa-IR')} <span className="text-sm font-normal">تومان</span></h3>
          
          <div className="w-full mt-8 space-y-3">
            <div className="flex justify-between items-center bg-white/10 p-4 rounded-2xl backdrop-blur-md">
              <div className="flex items-center gap-3"><TbTruckDelivery size={24}/> <span>اعتبار سفارش</span></div>
              <button onClick={() => setModalTopUp(true)} className="p-2 bg-white text-blue-600 rounded-xl shadow-lg"><FiPlus/></button>
            </div>
            <div className="flex justify-between items-center bg-white/10 p-4 rounded-2xl backdrop-blur-md">
              <div className="flex items-center gap-3"><FiMail size={24}/> <span>اعتبار پیامک: {smsBalance.toLocaleString('fa-IR')} عدد</span></div>
              <button onClick={() => setModalSms(true)} className="p-2 bg-white text-purple-600 rounded-xl shadow-lg"><FiPlus/></button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        <ActionButton icon={<FiPlus/>} label="افزایش اعتبار" color="bg-blue-600" onClick={() => setModalTopUp(true)}/>
        <ActionButton icon={<FiMail/>} label="بسته پیامک" color="bg-purple-600" onClick={() => setModalSms(true)}/>
        <ActionButton icon={<IoStatsChart/>} label="گزارش" color="bg-gray-800" onClick={() => {}}/>
      </div>

      {/* Transactions List */}
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-50">
        <h4 className="font-bold text-gray-800 mb-6 flex items-center gap-2">تراکنش‌های اخیر</h4>
        <div className="space-y-4">
          {[...txs].reverse().slice(0, 5).map(t => (
            <div key={t.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-2xl transition-all">
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-2xl ${t.balance_change > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                  {t.balance_change > 0 ? <ArrowUpCircle size={20}/> : <ArrowDownCircle size={20}/>}
                </div>
                <div>
                  <p className="font-bold text-gray-800 text-sm">{t.description}</p>
                  <p className="text-[10px] text-gray-400 mt-1">{new Date(t.timestamp).toLocaleDateString('fa-IR')}</p>
                </div>
              </div>
              <span className={`font-black ${t.balance_change > 0 ? 'text-green-600' : 'text-red-600'}`}>
                {Math.abs(t.balance_change / 10).toLocaleString('fa-IR')}
              </span>
            </div>
          ))}
        </div>
...
// } from '../companyService';
// import { WalletTransaction, SmsCreditTransaction, CompanyType } from '../../types';
// import { Loader2, ArrowDownCircle, ArrowUpCircle, X } from 'lucide-react';
// import PageLayout from '@/app/components/PageLayout';
// import { TbTruckDelivery } from 'react-icons/tb';
// import { FiMail, FiPlus } from 'react-icons/fi';
// import { IoStatsChart } from 'react-icons/io5';
